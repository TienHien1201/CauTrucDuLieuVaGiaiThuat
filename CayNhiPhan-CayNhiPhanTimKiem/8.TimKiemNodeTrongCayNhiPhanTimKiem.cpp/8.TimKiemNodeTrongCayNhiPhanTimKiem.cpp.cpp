...
1.Tìm phần tử có tồn tại trong cây hay không*/
...
//Hàm tìm kiếm 1 node nào đó có tồn tại trong cây hay không
node* timKiem(node* t, int x) {//Vì ta tìm là tìm 1 cái node nên kiểu dữ liệu phải là 1 cái node 
    if (t == NULL)
        return NULL;
    else
    {
        //Nếu phần tử cần tìm mà nhỏ hơn node gốc thì duyệt(Đệ quy) qua trái để tìm  
        if (x < t->data)
            timKiem(t->pLeft, x);
        //Nếu phần tử cần tìm mà lớn hơn node gốc thì duyệt(Đệ quy) qua phải để tìm  
        else if (x > t->data)
            timKiem(t->pRight, x);
        //Ta đã duyệt qua nhánh trái và nhánh phải của node gốc để tìm rồi nếu thấy thì ta trả về node đó luôn
        else //Tức là t->data == x; 
        {
            return t;
        }
    }
...
void Menu(node*& t) {
    while (true)
    {
        ...
        cout << "\n\n\t\t3.Kiem tra xem phan tu co ton tai trong cay khong";
        cout << "\n\n\t\t0.Ket Thuc ";
        ...
        {
            int x;
            cout << "Nhap x xem x co ton tai trong Cay khong: ";
            cin >> x;
            if (timKiem(t, x)!= NULL)
            {
                cout << "Co ton tai node " << x << " trong Cay !";
            }
            else
            {
                cout << "Khong ton tai node " << x << " trong cay !";
            }
            system("Pause");
        }
        else if (LuaChon == 0)
