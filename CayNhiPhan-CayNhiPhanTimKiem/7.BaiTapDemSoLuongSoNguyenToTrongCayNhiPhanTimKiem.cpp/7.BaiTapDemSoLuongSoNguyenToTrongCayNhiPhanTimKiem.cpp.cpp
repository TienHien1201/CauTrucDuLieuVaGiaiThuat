...
1.Đếm số lượng số nguyên tố trong cây nhị phân tìm kiếm */
...
//Hàm kiểm tra số nguyên tố. Tối ưu nhất 
bool isPrime(int x) {
    //Nếu x là số bé hơn 2 thì hàm trả về false luôn => Nghĩa là số đó không phải là số nguyên tố
    if (x < 2)
        return false;
    //Nếu x là số 2 thì hàm trả về true luôn => Nghĩa là số 2 là số nguyên tố
    if (x == 2)
        return true;
    /*Trường hợp duyệt all số chẳn vì ngoài số 2 ra tất cả số chẳn không phải là số nguyên tố => Nghĩa là khi gặp số chẳn thì hàm
    sẽ trả về fale tức số đó không phải là số nguyên tố*/
    if (x % 2 == 0)
        return false;

    /*Bây giờ chỉ còn số lẽ thì ta duyệt bắt đầu từ số 3 nếu x mà chia hết số nào ngoài 1 và chính nó thì không phải là số nguyên 
    tố*/
    int n = sqrt(x);
    for (int i = 3; i <= n; i += 2) {
        if (x % i == 0) {
...
void SoLuongSoNguyenToTrongCay(node* t, int &dem) {/*Tham chiếu biến dem để lưu kết quả đếm đc trong cây nhị phân có bao nhiêu
    node là số nguyên tố*/
    if (t != NULL)
    {
        //Xử lí 
        if (isPrime(t->data) == true)
        dem++;
        //2 dòng này có tác dụng là duyệt qua từng cái node trong cây
        SoLuongSoNguyenToTrongCay(t->pLeft, dem);
        SoLuongSoNguyenToTrongCay(t->pRight, dem);
    }
...
void Menu(node*& t) {
    while (true)
    {
        ...
        cout << "\n\n\t\t3.Kiem tra xem trong cay co bao nhieu so nguyen to";
        cout << "\n\n\t\t0.Ket Thuc ";
        ...
        {
            int dem = 0;
            SoLuongSoNguyenToTrongCay(t, dem);
            cout << "So Luong so nguyen to cua cay nhi phan la: " << dem;
